...
function init() {
  ...
  portfolioContainer = $("#portfolio-stocks");
  searchBox = $(".search-box");
  searchBox.keyup(() => throttle(search, 500));

  loadData("/data/stocks.csv")
    .then((results) => {
      recommendations = results.data;
      return recommendations;
    }).then((recs) => {
      renderData(recs, searchResultsContainer)
  });

  loadData("/data/portfolio-stocks.csv")
    .then((results) => {
      portfolioStocks = results.data;
      return portfolioStocks;
    }).then((stocks) => {
    renderData(stocks, portfolioContainer)
  });
}
...
function search() {
  ...
  if (searchText !== lastSearch) {
    ...
    renderData(filteredRecs, searchResultsContainer);
  }
}

function renderData(recs, container) {
  container.empty();
  for (let rec of recs) {
    container.append(makeStockHTML(rec));
  }
}
